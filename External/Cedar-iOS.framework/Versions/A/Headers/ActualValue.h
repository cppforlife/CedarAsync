...
#pragma mark class ActualValue
    ...
    private:
        ...
        void execute_positive_match(const MatcherType &) const;
        template<typename MatcherType>
        void execute_negative_match(const MatcherType &) const;
    ...
    template<typename T>
    ActualValue<T>::ActualValue(const char *fileName, int lineNumber, const T & value)
    : fileName_(fileName), lineNumber_(lineNumber), value_(value), to(*this), to_not(*this, true) {}
